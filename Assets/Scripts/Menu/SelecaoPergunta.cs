...
public class SelecaoPergunta : MonoBehaviour
{
    ...
    [SerializeField] private string[] fases;
    ...
    private void Update()
    {
        ...
        if (botaoSendoPressionado && botoes[botaoSelecionadoIndice].interactable)
        {
            ...
            if (tempoPressionado >= tempoPressionadoNecessario)
            {
                ...
                {
                    case 0:
                        StartCoroutine(carregaFase.IniciaFase(fases[0]));
                        carregandoFase = true;
                        break;
                    case 1:
                        StartCoroutine(carregaFase.IniciaFase(fases[1]));
                        carregandoFase = true;
                        break;
